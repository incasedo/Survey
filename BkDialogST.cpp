...
CBkDialogST::CBkDialogST(UINT uResourceID, CWnd* pParent)
	: cdxCDynamicDialog(uResourceID, pParent)
{
	Init();
}
...
CBkDialogST::CBkDialogST(LPCTSTR pszResourceID, CWnd* pParent)
	: cdxCDynamicDialog(pszResourceID, pParent)
{
	Init();
}
...
BEGIN_MESSAGE_MAP(CBkDialogST, cdxCDynamicDialog)
...
void CBkDialogST::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	cdxCDynamicDialog::OnLButtonDown(nFlags, point);
...
BOOL CBkDialogST::OnEraseBkgnd(CDC* pDC) 
{
	...
	BOOL	bRetValue = cdxCDynamicDialog::OnEraseBkgnd(pDC);
